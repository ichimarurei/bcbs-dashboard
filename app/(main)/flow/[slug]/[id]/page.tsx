...
const TableTransactionRecord = ({ params }: { params: Promise<{ slug: string; id: string }> }) => {
    ...
    const editBodyTemplate = (rowData: ICash) => <Button icon="pi pi-credit-card" outlined onClick={() => router.push(`/account/${rowData.id}`)} />;
            ...
            <div className="col-12 lg:col-6">
                <div className="p-3 h-full">
                    <div className="shadow-2 p-3 h-full flex flex-column" style={{ borderRadius: '6px' }}>
                        <div className="text-900 font-medium text-xl mb-2 uppercase">{infoCategories[category]}</div>
                        <div className="text-600">
                            {category === 'saving' ? 'Saldo Simpanan' : 'Sisa Cicilan'} <span className={`text-${category === 'saving' ? 'green' : 'orange'}-500 font-bold`}>{formatRp(finalCount)}</span>
                        </div>
                        ...
                        <Button label="Pendataan Baru" className="p-3 w-full mt-auto" severity={category === 'saving' ? 'success' : 'warning'} />
                    </div>
                </div>
            </div>
